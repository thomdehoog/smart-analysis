...
# ── Adversarial / weakness exposure ───────────────────────────


class TestAdversarial(unittest.TestCase):
    """Tests designed to expose weaknesses, race conditions, and
    incorrect assumptions in the engine. Every test here probes a
    specific failure mode that could occur in real usage."""

    def test_scope_complete_before_all_submits(self):
        """scope_complete called before all jobs are submitted.

        Only jobs submitted BEFORE scope_complete are collected.
        Jobs submitted after are lost. This tests that the engine
        behaves predictably (not deadlock or crash).
        """
        _temp_step("def run(pd, **p): pd['v'] = pd['input']['v']; return pd",
                   name="adv_early1")
        _temp_step("""
        ...
        with PipelineEngine() as e:
            ...
            # scope_complete with both submitted — should work
            r = run.scope_complete(spatial={"r": "X"}).result(timeout=30)
            self.assertEqual(r["n"], 2)

    def test_double_scope_complete_same_group(self):
        """Calling scope_complete twice for the same group.

        First call collects results and pops them. Second call finds
        nothing and should raise ScopeError.
        """
        _temp_step("def run(pd, **p): pd['v'] = 1; return pd",
                   name="adv_dsc1")
        _temp_step("def run(pd, **p): return pd", name="adv_dsc2")
        yaml = _temp_yaml("""
            wf:
              - adv_dsc1:
              - adv_dsc2:
        ...
        with PipelineEngine() as e:
            ...
            # Second call — results already consumed
            with self.assertRaises(ScopeError):
                run.scope_complete(spatial={"r": "X"}).result(timeout=10)

    def test_shared_input_dict_mutation(self):
        """User passes the same dict to multiple submits and mutates it.

        Each job should see the input_data as it was at submission time,
        not a later mutated version. This exposes reference-sharing bugs.
        """
        _temp_step("""
            def run(pd, **p):
                pd["saw"] = pd["input"]["value"]
                return pd
        """, name="adv_shared")
        yaml = _temp_yaml("wf:\n  - adv_shared:")
        ...
        with PipelineEngine() as e:
            run = e.create_run(yaml)
            data = {"value": "original"}
            f1 = run.submit("j1", data)
            data["value"] = "mutated"  # mutate after submit
            f2 = run.submit("j2", data)

            r1 = f1.result(timeout=10)
            r2 = f2.result(timeout=10)

        # r1 might see "original" or "mutated" depending on race.
        # r2 should always see "mutated".
        # The key point: no crash, no deadlock.
        self.assertEqual(r2["saw"], "mutated")
        # Note: r1["saw"] is non-deterministic — it depends on
        # whether the executor thread reads input_data before or after
        # the mutation. This is a known weakness documented here.

    def test_step_corrupts_metadata(self):
        """Step modifies pipeline_data['metadata'] — does the engine break?"""
        _temp_step("""
            def run(pd, **p):
                pd["metadata"]["corrupted"] = True
                pd["metadata"]["verbose"] = 999
                return pd
        """, name="adv_meta1")
        _temp_step("""
            def run(pd, **p):
                pd["saw_corrupt"] = pd["metadata"].get("corrupted")
                return pd
        """, name="adv_meta2")
        yaml = _temp_yaml("wf:\n  - adv_meta1:\n  - adv_meta2:")

        from engine import run_pipeline
        # Should not crash — metadata corruption is the step's problem
        r = run_pipeline(yaml, "t", {})
        self.assertTrue(r["saw_corrupt"])

    def test_step_returns_input_unmodified(self):
        """Step returns pipeline_data without adding anything."""
        _temp_step("def run(pd, **p): return pd", name="adv_noop1")
        _temp_step("""
            def run(pd, **p):
                pd["after_noop"] = True
                return pd
        ...
        from engine import run_pipeline
        r = run_pipeline(yaml, "t", {})
        self.assertTrue(r["after_noop"])

    def test_step_replaces_pipeline_data_entirely(self):
        """Step returns a completely new dict, discarding all prior data."""
        _temp_step("def run(pd, **p): pd['s1'] = True; return pd",
                   name="adv_replace1")
        _temp_step("def run(pd, **p): return {'replaced': True}",
                   name="adv_replace2")
        _temp_step("""
            def run(pd, **p):
                pd["saw_s1"] = pd.get("s1")
                pd["saw_replaced"] = pd.get("replaced")
                return pd
        """, name="adv_replace3")
        yaml = _temp_yaml(
            "wf:\n  - adv_replace1:\n  - adv_replace2:\n  - adv_replace3:")
        ...
        from engine import run_pipeline
        r = run_pipeline(yaml, "t", {})
        self.assertIsNone(r["saw_s1"])  # lost when step 2 replaced
        self.assertTrue(r["saw_replaced"])

    def test_process_cleanup_after_worker_error(self):
        """After a worker error, no orphan processes or leaked sockets."""
        from engine._worker import Worker
        path = _temp_step('def run(pd, **p): raise ValueError("crash")')
    ...
    def test_process_cleanup_after_crash(self):
        """After a process crash, no orphan processes."""
        from engine._worker import Worker
        path = _temp_step("import os\ndef run(pd, **p): os._exit(1)")
    ...
    def test_pool_cleanup_after_errors(self):
        """Pool cleans up all workers on shutdown, even after errors."""
        from engine._pool import WorkerPool
        path_ok = _temp_step("def run(pd, **p): pd['ok'] = True; return pd")
        path_bad = _temp_step('def run(pd, **p): raise ValueError("x")')

        pool = WorkerPool()
        pool.execute("local", "cpu", path_ok, {}, {}, timeout=10)
        try:
            pool.execute("local", "cpu", path_bad, {}, {}, timeout=10)
        except StepExecutionError:
            pass
        pool.shutdown_all()

        self.assertEqual(len(pool._cpu_workers), 0)
        self.assertEqual(len(pool._cpu_locks), 0)

    def test_shutdown_during_active_work(self):
        """Engine shutdown while jobs are running — no hang, no crash."""
        _temp_step("""
            import time
            def run(pd, **p):
                time.sleep(2)
        ...
        # Shutdown while job is running — should not hang
        e.shutdown(wait=False)
        self.assertFalse(e._accepting)

    def test_reuse_run_for_multiple_batches(self):
        """Submit multiple batches of jobs to the same run."""
        _temp_step("""
            def run(pd, **p):
                pd["batch"] = pd["input"]["batch"]
                return pd
        ...
        with PipelineEngine() as e:
            ...
            # Batch 1
            f1 = [run.submit(f"b1_{i}", {"batch": 1}) for i in range(3)]
            r1 = [f.result(timeout=10) for f in f1]

            # Batch 2 (same run object)
            f2 = [run.submit(f"b2_{i}", {"batch": 2}) for i in range(3)]
    ...
    def test_concurrent_submits_to_same_scope_group(self):
        """Many threads submitting to the same scope group simultaneously."""
        _temp_step("def run(pd, **p): pd['v'] = pd['input']['v']; return pd",
                   name="adv_conc_scope1")
        _temp_step("""
            def run(pd, **p):
                pd["values"] = sorted(r["v"] for r in pd["results"])
                return pd
        """, name="adv_conc_scope2")
        yaml = _temp_yaml("""
            wf:
              - adv_conc_scope1:
              - adv_conc_scope2:
        ...
        with PipelineEngine(max_concurrent=n) as e:
            ...
            def submit_job(i):
                f = run.submit(f"j{i}", {"v": i}, spatial={"r": "X"})
                with lock:
                    futures.append(f)
    ...
    def test_step_with_very_slow_import(self):
        """Step that takes long to import — worker connect timeout matters."""
        _temp_step("""
            import time
            time.sleep(0.5)  # slow module-level code

            def run(pd, **p):
                pd["loaded"] = True
                return pd
        """, name="adv_slow_import")
        yaml = _temp_yaml("wf:\n  - adv_slow_import:")
    ...
    def test_yaml_with_duplicate_step_names(self):
        """Same step used twice in a pipeline — both invocations run.

        Each invocation gets a fresh module (load_function creates new
        module per call). The test verifies both run and params are
        delivered correctly, not that module state is shared.
        """
        _temp_step("""
            def run(pd, **p):
                tag = p.get("tag", "none")
                pd.setdefault("tags", []).append(tag)
                return pd
        """, name="adv_dup")
        yaml = _temp_yaml("""
            wf:
        ...
        from engine import run_pipeline
        r = run_pipeline(yaml, "t", {})
        self.assertEqual(r["tags"], ["first", "second"])

    def test_empty_scope_labels(self):
        """Submit with empty scope dicts — should work like no scopes."""
        _temp_step("def run(pd, **p): pd['ok'] = True; return pd",
                   name="adv_empty_scope")
        yaml = _temp_yaml("wf:\n  - adv_empty_scope:")

        from engine import PipelineEngine
        with PipelineEngine() as e:
            run = e.create_run(yaml)
            r = run.submit("j", {}, spatial={}, temporal={}).result(timeout=10)
        self.assertTrue(r["ok"])

    def test_scope_labels_with_special_characters(self):
        """Scope labels with spaces, unicode, and special chars."""
        _temp_step("def run(pd, **p): pd['v'] = 1; return pd",
                   name="adv_special1")
        _temp_step("""
            def run(pd, **p):
        ...
        with PipelineEngine() as e:
            run = e.create_run(yaml)
            run.submit("j1", {},
                       spatial={"region": "R with spaces"})
            run.submit("j2", {},
                       spatial={"region": "R with spaces"})
            r = run.scope_complete(
                spatial={"region": "R with spaces"}).result(timeout=10)

        self.assertEqual(r["n"], 2)

    def test_deeply_nested_pipeline_data(self):
        """Deeply nested dicts survive through the pipeline."""
        _temp_step("""
            def run(pd, **p):
                d = {"level": 0}
                current = d
        ...
        from engine import run_pipeline
        r = run_pipeline(yaml, "t", {})
        self.assertEqual(r["max_depth"], 49)

    def test_multiple_runs_same_yaml(self):
        """Two runs created from the same YAML are fully independent."""
        _temp_step("""
            def run(pd, **p):
                pd["v"] = pd["input"]["v"]
                return pd
        ...
        from engine import PipelineEngine
        with PipelineEngine() as e:
            run_a = e.create_run(yaml)
            run_b = e.create_run(yaml)

            run_a.submit("a1", {"v": 1}, spatial={"r": "X"})
            run_a.submit("a2", {"v": 2}, spatial={"r": "X"})
            run_b.submit("b1", {"v": 100}, spatial={"r": "X"})

            ra = run_a.scope_complete(spatial={"r": "X"}).result(timeout=30)
            rb = run_b.scope_complete(spatial={"r": "X"}).result(timeout=30)

        # Results must not cross between runs
        self.assertEqual(ra["total"], 3)
        self.assertEqual(rb["total"], 100)

    def test_max_concurrent_1_no_deadlock(self):
        """Engine with max_concurrent=1 and scoped pipeline — no deadlock.

        With 1 thread: Phase 0 jobs queue, then scope_complete queues.
        The executor processes them in order. scope_complete must not
        deadlock waiting for Phase 0 futures that haven't started yet.
        """
        _temp_step("def run(pd, **p): pd['v'] = pd['input']['v']; return pd",
                   name="adv_mc1_a")
        _temp_step("""
        ...
        with PipelineEngine(max_concurrent=1) as e:
            ...
            # All 3 tasks (j1, j2, scope_complete) go through 1 thread
            r = run.scope_complete(spatial={"r": "X"}).result(timeout=30)

        self.assertEqual(r["sum"], 30)

    def test_deterministic_output(self):
        """Same inputs produce same outputs across repeated runs."""
        _temp_step("""
            def run(pd, **p):
                data = pd["input"]["data"]
                pd["result"] = sum(x ** 2 for x in data)
                return pd
        """, name="adv_det")
        yaml = _temp_yaml("wf:\n  - adv_det:")
        ...
        results = []
        for _ in range(5):
            r = run_pipeline(yaml, "t", {"data": [1, 2, 3, 4, 5]})
